...
from utils.PyGRDECL.GRDECL_Parser import *
from utils.PyGRDECL.GRDECL_FaultProcess import *
from utils.PyGRDECL.utils import *
...
class GeologyModel:
    ...
    def CreateCellData(self, varname="SW", val=0.0, val_array=[]):
        ...
        Date: Feb. 2018
        """
        assert varname not in self.GRDECL_Data.SpatialDatas, "     [Error] Variable [%s] is existed! Please use UpdateCellData function" % (
            varname)
        ...
        if (len(val_array) == 0):
            ...
            assert len(val_array) == self.GRDECL_Data.N, print(
                '     [Error] Input array is not compatible with number of cells!')
    ...
    def LoadCellData(self, varname="SW", filename="123.txt"):
        ...
        if (Data is not None):
            assert len(Data) == self.GRDECL_Data.N, print(
                '     [Error] Input array is not compatible with number of cells!')
                    ...
                    if (i >= nx_range[0] and i <= nx_range[1]):
                        if (j >= ny_range[0] and j <= ny_range[1]):
                            if (k >= nz_range[0] and k <= nz_range[1]):
                                ...
                                if (
                                        len(array) > 0):  # We load up value from a 1D array which storaged in the following loop order
                                    val = array[ijk]
    ...
    def Write2VTP(self, outFile=None):
        self.Write2VTU(outFile.replace('.vtp', '.vtu'))
        basename = os.path.splitext(os.path.basename(self.fname))[0]
        if outFile is None:
            outFile = os.path.join('Results', basename + '.vtp')
        inputFile = outFile.replace('.vtp', '.vtu')
